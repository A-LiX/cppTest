...
// 交易所特定的配置工厂
namespace ExchangeConfigs {
    // Binance 配置
    inline GenericWebSocketClient::Config binance_spot(const std::string& stream) {
        return GenericWebSocketClient::Config(
            "stream.binance.com", 
            "443", 
            "/ws/" + stream,
            true,  // use_ssl
    ...
    inline GenericWebSocketClient::Config binance_futures(const std::string& stream) {
        return GenericWebSocketClient::Config(
            "fstream.binance.com", 
            "443", 
            "/ws/" + stream,
    ...
    // Coinbase 配置
    inline GenericWebSocketClient::Config coinbase() {
        return GenericWebSocketClient::Config(
    ...
    // Kraken 配置
    inline GenericWebSocketClient::Config kraken() {
        return GenericWebSocketClient::Config(
            "ws.kraken.com", 
            "443", 
            "/",
...
public:
    void process(const std::string& message) {
        try {
            ...
            // 处理深度更新事件
            else if (json_data.as_object().contains("e") && 
                     json_data.at("e").as_string() == "depthUpdate") {
                std::string symbol = json_data.at("s").as_string().c_str();
                uint64_t event_time = json_data.at("E").as_int64();
                
                std::cout << "[Binance Depth] " << symbol 
                          << " | Event time: " << event_time << std::endl;
            }
            // 处理其他消息
            else {
                std::cout << "[Binance Other] " << message.substr(0, 80) 
                          << (message.size() > 80 ? "..." : "") << std::endl;
            }
        } catch (const std::exception& e) {
            std::cerr << "Binance JSON error: " << e.what() 
...
public:
    void process(const std::string& message) {
        try {
            ...
            std::cerr << "Coinbase JSON error: " << e.what() 
                      << "\nMessage: " << message << std::endl;
        }
    }
};
...
int main() {
    try {
        ...
        {
            auto config = ExchangeConfigs::binance_spot("btcusdt@trade");
            auto client = std::make_shared<GenericWebSocketClient>(ioc, config);
            
            auto processor = std::make_shared<BinanceMessageProcessor>();
            
            client->set_message_handler([processor](const std::string& msg) {
                processor->process(msg);
            });
            
            client->set_error_handler([](const std::string& error) {
                std::cerr << "Binance Error: " << error << std::endl;
            });
        ...
        // 示例 2: 连接 Coinbase
        {
            auto config = ExchangeConfigs::coinbase();
            auto client = std::make_shared<GenericWebSocketClient>(ioc, config);
            
            auto processor = std::make_shared<CoinbaseMessageProcessor>();
            ...
            client->set_error_handler([](const std::string& error) {
                std::cerr << "Coinbase Error: " << error << std::endl;
            });
